...
const SKILLFUL_VERSION = readSkillfulVersion();
const SKILLFUL_BRAND_IMAGE_URL = 'https://primeradiant.com/brand/superpowers-visual-brainstorming-logo.png';
const TELEMETRY_DISABLE_ENV_VARS = [
  'SUPERPOWERS_DISABLE_TELEMETRY',
  'DISABLE_TELEMETRY',
  'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'
];
const SKILLFUL_TELEMETRY_DISABLED = TELEMETRY_DISABLE_ENV_VARS.some(name => isTruthyEnv(process.env[name]));
...
function brandMarkup() {
  const version = escapeHtmlText(SKILLFUL_VERSION);
  const text = SKILLFUL_TELEMETRY_DISABLED
    ? 'Skillful v' + version
    : 'Skillful v' + version;
  const logo = SKILLFUL_TELEMETRY_DISABLED
    ? ''
    : '<img class="brand-logo" src="' + SKILLFUL_BRAND_IMAGE_URL + '?v=' + encodeURIComponent(SKILLFUL_VERSION) + '" alt="Skillful" referrerpolicy="no-referrer" decoding="async">';
